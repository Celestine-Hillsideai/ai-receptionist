...
- **[daily-digest.json](daily-digest.json)** — Workflow D. Cron `0 8 * * *` (daily 08:00 UTC — not yet business-hours/timezone aware, see spec §64; `/api/daily-summary`'s own date default is WAT-aware, see `lib/timezone.ts`). Calls `GET /api/daily-summary`, then **"Has Calls Today?" skips the send entirely when `totalCalls` is 0** (added after the digest was found emailing an empty "0 calls" summary every day, including days with zero calls) — otherwise formats the spec §27 digest text and sends it. **Active.**
