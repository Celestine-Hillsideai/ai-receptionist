...
- **Admin dashboard** (Phase 5): Dashboard, Calls (search/filter/paginate), Call Detail (transcript, structured analysis, follow-up status/notes), Daily Summary, Settings — see `app/dashboard/`. `GET /api/calls` and `GET /api/calls/:id` added per spec §39. **No dashboard login yet** — read `docs/DEPLOYMENT.md` before deploying anywhere public.
